...
-- 사수에 해당하는 직원에 대해 조회
-- 사번, 이름, 부서명, 직급급명, 구분(사수/사원)

-- * 사수 = MANAGER_ID 칼럼에 작성된 사번

SELECT * FROM EMPLOYEE;


-- 1) 사수에 해당하는 사원번번호 조회 (서브쿼리)
SELECT  DISCONNECT  MANAGER-ID
FROM EMPLOYEE 
SELECT MANAGER-ID IS BOT NULL;
...
SELECT EMP_NAME, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리';
...
-- 2) - DEPARTMENT 테이블의 결과(L1)와 동일한 LOCATION_ID를 가지고 있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 상관쿼리 (이태림이 D8부서에서 가장 빠른입사/퇴사자 걸러짐 - D8 부서 아예 제외)	
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 퇴사자인 이태림을 제외한 상태로, D8 부서의 가장 빠른 입사자도 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
FROM (SELECT EMP_NAME, SALARY
		FROM EMPLOYEE
		ORDER BY SALARY DESC) -- 해석1순위 FROM 절에서 전체 직원의 SALARY 내림차순 정렬
...
SELECT EMP_ID, EMP_NAME, PHONE, TO_CHAR(HIRE_DATE,'RR/MM/DD') "HIRE DATE", DEPT_TITLE
FROM EMPLOYEE
...
WITH EX2 AS(SELECT EMP_ID, EMP_NAME, PHONE, SALARY, JOB_NAME
				FROM EMPLOYEE
				JOIN JOB USING(JOB_CODE)
...
FROM EX2
WHERE SAlARY = (SELECT CEIL (MAX(SAlARY))
				FROM EX2);
