...
-- 문자열 타입이여도 저장된 값이 숫자면 자동으로 형변환하여 연산 가능
 ...
 * 
 * 컬럼명 AS "별칭" : 병칭 띄어쓰기 O, 특수문자 O, 문자만 O
 * 
 * AS 생략 가능
 * 
 * */
...
-- EMPLOYEE 테이블에서 EMAIL의 _ 앞에 글자가 세글자인 사원만
-- 이름, 이메일 조회
...
---EMPLOYEE 테이블에서 보너스가없는 사원의 이름, 보너스 조회
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
...
-- EMPLOYEE 테이블 급여 (FROM 절)
-- 급여 오름 차순으로(ODER BY 절)
...
ORDER BY SALARY; ASC(오름차순) 가 기본값 [ASC | DESC]
 ...
 -- EMPLOYEE 테이블에서 (FROM 절)
 -- 급여가 200만의 사원의 (WHERE 절)
 -- 사번, 이름, 급여 조회( SELECET 절)
 -- 단, 급여 내림 차순르로 조회(ORDER BY 절)
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
